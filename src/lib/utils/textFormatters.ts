export function formatLocationCoverage(city: string | undefined, areas: string[]): string {
    ...
    if (!areas || areas.length === 0) {
        return `Local Locksmith near ${safeCity} and surrounding areas.`;
    }

    if (areas.length === 1) {
        return `Local Locksmith near ${safeCity}, covering ${areas[0]} and surrounding areas.`;
    }

    const last = areas[areas.length - 1];
    const rest = areas.slice(0, -1).join(', ');

    return `Local Locksmith near ${safeCity}, covering ${rest} and ${last}.`;
}
